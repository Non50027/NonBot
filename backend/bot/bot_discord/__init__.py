...
class Bot(commands.Bot):
    ...
    async def load_extensions(self):
        ...
        for file in file_list:
            try:
                temp_file_list= file.split('\\')
                if temp_file_list[-1].startswith('temp_file_list'): 
                    print(f'    \033[1;32m-\033[0m {temp_file_list[-1][:-3]} ... 未完成的檔案', end='')
                    continue
                print(f'    \033[1;32m-\033[0m {temp_file_list[-1][:-3]} ... ', end='')
                temp_file_list= file.split('\\bot\\')[-1].split('\\')
                await self.load_extension('.'.join(temp_file_list)[:-3])
