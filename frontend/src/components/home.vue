<template>
  <div>
    <h1>農農的小烏龜</h1>
    <div v-if="discord_login.login" class="alert alert-success" role="alert">
        Discord 上線中
    </div>
    <div v-else class="alert alert-danger" role="alert">
      Discord 離線
    </div>
    <div v-if="twitch_login.login" class="alert alert-success" role="alert">
      twitch 上線中
    </div>
    <div v-else class="alert alert-danger" role="alert">
      Twitch 離線
...
onBeforeMount(()=>{
    // 取得 discord 狀態
    axios.get(`${import.meta.env.VITE_BACKEND_URL}/discord/state`)
    .then(response => {
    ...
    // 取得 twitch 狀態
    axios.get(`${import.meta.env.VITE_BACKEND_URL}/twitch/state`)
    .then(response => {
      console.log(response.data)
      Object.assign(twitch_login, response.data)
    })
    .catch(error => {
        console.log(error.response)
    })
